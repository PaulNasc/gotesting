...
export class DatabaseMigrationService {
  ...
  private static async createAccessTokensTable(client: SupabaseClient) {
    ...
    if (error && error.code === '42P01') {
      throw new Error('Access tokens table does not exist. Please run the SQL migrations.');
    }
  }
  ...
  private static async createAccessLogsTable(client: SupabaseClient) {
    ...
    if (error && error.code === '42P01') {
      throw new Error('Access logs table does not exist. Please run the SQL migrations.');
    }
  }
